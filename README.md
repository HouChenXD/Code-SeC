...
4. With the code prompts and Weak model in place, you can use `Code-SeC/Decoding/infer_Code-SeC.py` in `Decoding`. Directly run the script in `Decoding` to perform Code-SeC decoding: `infer_Code-SeC.sh`. Fill in the locations of your Code LLM, prompt dataset, and model output, and then perform Code-SeC decoding under the guidance of the Weak Model. You can also change the hyperparameters in `Decoding/infer_Code-SeC.py` to adjust the decoding process.
